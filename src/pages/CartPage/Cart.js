...
const Cart = () => {
  ...
  const ordersRendering = states.cart.products.map((product, index) => {
    return <RestaurantProductCard key={index} product={product} />;
  });

  const ordersPrice = states?.cart?.products.reduce(
  ...
  const placeOrder = () => {
    ...
    if (states.cart.products.length > 0) {
      axios
        .post(
          `https://us-central1-missao-newton.cloudfunctions.net/futureEatsA/restaurants/${states.cart.restaurantId}/order`,
          body,
          headers
        )
        .then((res) => {
  ...
  return (
    <div>
      ...
      <PaymentMethod>
        {states.cart.products.length > 0 ? ordersRendering : "Carrinho vazio"}
      </PaymentMethod>

      <Main>
        <Total>
          <TextTotal>
            {states.cart.products.length > 0
              ? `Frete R$ ${states.cart.shipping.toFixed(2)}`
              : "Frete R$ 0.00"}
          </TextTotal>
          <SubTotal>
            ...
            <TextTotal>
              {states.cart.products.length > 0
                ? `R$ ${(ordersPrice + states.cart.shipping).toFixed(2)}`
                : "R$ 0.00"}
            </TextTotal>
          </SubTotal>
        </Total>
